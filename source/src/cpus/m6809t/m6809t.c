...
#define FETCH8(s)   m6809t_read(s, (s)->PC++)
#define FETCH_OP(s) m6809t_read_opcode(s, (s)->PC++)

static inline uint16_t FETCH16(m6809t_state *s) {
    uint8_t hi = m6809t_read(s, s->PC++);
    uint8_t lo = m6809t_read(s, s->PC++);
    return ((uint16_t)hi << 8) | lo;
}

#define READ8(s,a)    m6809t_read(s, a)

static inline uint16_t READ16(m6809t_state *s, uint16_t a) {
    uint8_t hi = m6809t_read(s, a);
    uint8_t lo = m6809t_read(s, (uint16_t)(a + 1));
    return ((uint16_t)hi << 8) | lo;
}

#define WRITE8(s,a,v) m6809t_write(s, a, v)
#define WRITE16(s,a,v) do { m6809t_write(s,a,(uint8_t)((v)>>8)); m6809t_write(s,(uint16_t)((a)+1),(uint8_t)(v)); } while(0)

#define PUSH8(s,v)  do { (s)->S--; WRITE8(s, (s)->S, v); } while(0)
#define PUSH16(s,v) do { PUSH8(s, (uint8_t)(v)); PUSH8(s, (uint8_t)((v)>>8)); } while(0)
#define PULL8(s)    m6809t_read(s, (s)->S++)

static inline uint16_t PULL16(m6809t_state *s) {
    uint8_t hi = m6809t_read(s, s->S++);
    uint8_t lo = m6809t_read(s, s->S++);
    return ((uint16_t)hi << 8) | lo;
}

#define PUSHU8(s,v)  do { (s)->U--; WRITE8(s, (s)->U, v); } while(0)
#define PUSHU16(s,v) do { PUSHU8(s, (uint8_t)(v)); PUSHU8(s, (uint8_t)((v)>>8)); } while(0)
#define PULLU8(s)    m6809t_read(s, (s)->U++)

static inline uint16_t PULLU16(m6809t_state *s) {
    uint8_t hi = m6809t_read(s, s->U++);
    uint8_t lo = m6809t_read(s, s->U++);
    return ((uint16_t)hi << 8) | lo;
}

/* CC flag helpers */
#define SET_Z8(s,v)   do { if(!(uint8_t)(v)) (s)->CC |= M6809T_CC_Z; else (s)->CC &= ~M6809T_CC_Z; } while(0)
#define SET_Z16(s,v)  do { if(!(uint16_t)(v)) (s)->CC |= M6809T_CC_Z; else (s)->CC &= ~M6809T_CC_Z; } while(0)
#define SET_N8(s,v)   do { if((v)&0x80) (s)->CC |= M6809T_CC_N; else (s)->CC &= ~M6809T_CC_N; } while(0)
#define SET_N16(s,v)  do { if((v)&0x8000) (s)->CC |= M6809T_CC_N; else (s)->CC &= ~M6809T_CC_N; } while(0)

static void set_nz8(m6809t_state *s, uint8_t v) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z);
    if (!v) s->CC |= M6809T_CC_Z;
    if (v & 0x80) s->CC |= M6809T_CC_N;
}

static void set_nz16(m6809t_state *s, uint16_t v) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z);
    if (!v) s->CC |= M6809T_CC_Z;
    if (v & 0x8000) s->CC |= M6809T_CC_N;
}
...
static uint16_t *get_index_reg(m6809t_state *s, uint8_t postbyte) {
    switch ((postbyte >> 5) & 3) {
        case 0: return &s->X;
        case 1: return &s->Y;
        case 2: return &s->U;
...
static uint16_t indexed_addr(m6809t_state *s) {
...
static uint16_t direct_addr(m6809t_state *s) {
    return ((uint16_t)s->DP << 8) | FETCH8(s);
}

static uint16_t extended_addr(m6809t_state *s) {
    return FETCH16(s);
}
...
static uint8_t op_add8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint16_t r = a + b;
    s->CC &= ~(M6809T_CC_H | M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (((a ^ b ^ r) & 0x10)) s->CC |= M6809T_CC_H;
    if (r & 0x80) s->CC |= M6809T_CC_N;
    if (!(r & 0xFF)) s->CC |= M6809T_CC_Z;
    if (((a ^ b ^ 0xFF) & (a ^ r)) & 0x80) s->CC |= M6809T_CC_V;
    if (r & 0x100) s->CC |= M6809T_CC_C;
    return (uint8_t)r;
}

static uint8_t op_adc8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint16_t r = a + b + (s->CC & M6809T_CC_C ? 1 : 0);
    s->CC &= ~(M6809T_CC_H | M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (((a ^ b ^ r) & 0x10)) s->CC |= M6809T_CC_H;
    if (r & 0x80) s->CC |= M6809T_CC_N;
    if (!(r & 0xFF)) s->CC |= M6809T_CC_Z;
...
static uint8_t op_sub8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint16_t r = a - b;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (r & 0x80) s->CC |= M6809T_CC_N;
    if (!(r & 0xFF)) s->CC |= M6809T_CC_Z;
    if (((a ^ b) & (a ^ r)) & 0x80) s->CC |= M6809T_CC_V;
    if (r & 0x100) s->CC |= M6809T_CC_C;
    return (uint8_t)r;
}

static uint8_t op_sbc8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint16_t r = a - b - (s->CC & M6809T_CC_C ? 1 : 0);
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (r & 0x80) s->CC |= M6809T_CC_N;
    if (!(r & 0xFF)) s->CC |= M6809T_CC_Z;
...
static void op_cmp8(m6809t_state *s, uint8_t a, uint8_t b) {
    op_sub8(s, a, b); /* discard result */
}

static uint8_t op_and8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint8_t r = a & b;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    set_nz8(s, r);
    return r;
}

static uint8_t op_or8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint8_t r = a | b;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    set_nz8(s, r);
    return r;
...
static uint8_t op_eor8(m6809t_state *s, uint8_t a, uint8_t b) {
    uint8_t r = a ^ b;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    set_nz8(s, r);
    return r;
...
static void op_bit8(m6809t_state *s, uint8_t a, uint8_t b) {
    op_and8(s, a, b); /* discard result */
}

static void op_tst8(m6809t_state *s, uint8_t v) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    set_nz8(s, v);
}
...
static uint16_t op_add16(m6809t_state *s, uint16_t a, uint16_t b) {
    uint32_t r = (uint32_t)a + b;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (r & 0x8000) s->CC |= M6809T_CC_N;
    if (!(r & 0xFFFF)) s->CC |= M6809T_CC_Z;
    if (((a ^ b ^ 0xFFFF) & (a ^ r)) & 0x8000) s->CC |= M6809T_CC_V;
    if (r & 0x10000) s->CC |= M6809T_CC_C;
    return (uint16_t)r;
}

static uint16_t op_sub16(m6809t_state *s, uint16_t a, uint16_t b) {
    uint32_t r = (uint32_t)a - b;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (r & 0x8000) s->CC |= M6809T_CC_N;
    if (!(r & 0xFFFF)) s->CC |= M6809T_CC_Z;
    if (((a ^ b) & (a ^ r)) & 0x8000) s->CC |= M6809T_CC_V;
    if (r & 0x10000) s->CC |= M6809T_CC_C;
    return (uint16_t)r;
}

static void op_cmp16(m6809t_state *s, uint16_t a, uint16_t b) {
    op_sub16(s, a, b);
}

/* Shift/Rotate operations */

static uint8_t op_asl8(m6809t_state *s, uint8_t v) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (v & 0x80) s->CC |= M6809T_CC_C;
    uint8_t r = v << 1;
    set_nz8(s, r);
    if (((v ^ r) & 0x80)) s->CC |= M6809T_CC_V;
    return r;
}

static uint8_t op_asr8(m6809t_state *s, uint8_t v) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_C);
    if (v & 0x01) s->CC |= M6809T_CC_C;
    uint8_t r = (v >> 1) | (v & 0x80);
    set_nz8(s, r);
    return r;
}

static uint8_t op_lsr8(m6809t_state *s, uint8_t v) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_C);
    if (v & 0x01) s->CC |= M6809T_CC_C;
    uint8_t r = v >> 1;
    set_nz8(s, r);
...
static uint8_t op_rol8(m6809t_state *s, uint8_t v) {
    uint8_t c = s->CC & M6809T_CC_C ? 1 : 0;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    if (v & 0x80) s->CC |= M6809T_CC_C;
    uint8_t r = (v << 1) | c;
    set_nz8(s, r);
...
static uint8_t op_ror8(m6809t_state *s, uint8_t v) {
    uint8_t c = s->CC & M6809T_CC_C ? 0x80 : 0;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_C);
    if (v & 0x01) s->CC |= M6809T_CC_C;
    uint8_t r = (v >> 1) | c;
    set_nz8(s, r);
...
static uint8_t op_neg8(m6809t_state *s, uint8_t v) {
    uint8_t r = (uint8_t)(0 - v);
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V | M6809T_CC_C);
    set_nz8(s, r);
    if (v == 0x80) s->CC |= M6809T_CC_V;
    if (r) s->CC |= M6809T_CC_C;
    return r;
}

static uint8_t op_com8(m6809t_state *s, uint8_t v) {
    uint8_t r = ~v;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    s->CC |= M6809T_CC_C;
    set_nz8(s, r);
...
static uint8_t op_inc8(m6809t_state *s, uint8_t v) {
    uint8_t r = v + 1;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    set_nz8(s, r);
    if (v == 0x7F) s->CC |= M6809T_CC_V;
    return r;
}

static uint8_t op_dec8(m6809t_state *s, uint8_t v) {
    uint8_t r = v - 1;
    s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
    set_nz8(s, r);
    if (v == 0x80) s->CC |= M6809T_CC_V;
...
static uint8_t op_clr8(m6809t_state *s) {
    s->CC &= ~(M6809T_CC_N | M6809T_CC_V | M6809T_CC_C);
    s->CC |= M6809T_CC_Z;
    return 0;
}

/* Branch condition check */
static int branch_cond(m6809t_state *s, uint8_t cond) {
    switch (cond & 0x0F) {
        case 0x0: return 1;                                         /* BRA */
        case 0x1: return 0;                                         /* BRN */
        case 0x2: return !(s->CC & (M6809T_CC_C | M6809T_CC_Z));     /* BHI */
        case 0x3: return  (s->CC & (M6809T_CC_C | M6809T_CC_Z)) != 0;/* BLS */
        case 0x4: return !(s->CC & M6809T_CC_C);                    /* BCC/BHS */
        case 0x5: return  (s->CC & M6809T_CC_C) != 0;               /* BCS/BLO */
        case 0x6: return !(s->CC & M6809T_CC_Z);                    /* BNE */
        case 0x7: return  (s->CC & M6809T_CC_Z) != 0;               /* BEQ */
        case 0x8: return !(s->CC & M6809T_CC_V);                    /* BVC */
        case 0x9: return  (s->CC & M6809T_CC_V) != 0;               /* BVS */
        case 0xA: return !(s->CC & M6809T_CC_N);                    /* BPL */
        case 0xB: return  (s->CC & M6809T_CC_N) != 0;               /* BMI */
        case 0xC: return !((s->CC & M6809T_CC_N) != 0) == !((s->CC & M6809T_CC_V) != 0); /* BGE */
        case 0xD: return  ((s->CC & M6809T_CC_N) != 0) != ((s->CC & M6809T_CC_V) != 0);  /* BLT */
        case 0xE: return !(s->CC & M6809T_CC_Z) &&                  /* BGT */
                         (!!(s->CC & M6809T_CC_N) == !!(s->CC & M6809T_CC_V));
        case 0xF: return  (s->CC & M6809T_CC_Z) ||                  /* BLE */
                         (!!(s->CC & M6809T_CC_N) != !!(s->CC & M6809T_CC_V));
    }
    return 0;
}
...
static uint16_t get_reg(m6809t_state *s, uint8_t code) {
    switch (code & 0x0F) {
...
static void set_reg(m6809t_state *s, uint8_t code, uint16_t val) {
    switch (code & 0x0F) {
...
static void push_entire_state(m6809t_state *s) {
    s->CC |= M6809T_CC_E;
    PUSH16(s, s->PC);
    PUSH16(s, s->U);
    PUSH16(s, s->Y);
...
static void check_interrupts(m6809t_state *s) {
    ...
    if (s->irq_pending && !(s->CC & M6809T_CC_I)) {
        s->irq_pending = 0;
        /* CWAI (halted==2) already pushed state; skip re-push */
        if (s->halted != 2) push_entire_state(s);
        s->halted = 0;
...
void m6809t_reset(m6809t_state *s) {
    s->A = s->B = 0;
    s->X = s->Y = s->U = s->S = 0;
    s->DP = 0;
    s->CC = M6809T_CC_I | M6809T_CC_F;
...
void m6809t_irq(m6809t_state *s) {
    s->irq_pending = 1;
    check_interrupts(s);
...
void m6809t_firq(m6809t_state *s) {
    s->firq_pending = 1;
}

void m6809t_nmi(m6809t_state *s) {
    s->nmi_pending = 1;
}
...
int m6809t_step(m6809t_state *s, char count) {
  ...
  for (int i=0; i < count; i++) {
    ...
    switch (op) {
        ...
        case 0x19: /* DAA */
        {
            ...
            if (lsn > 9 || (s->CC & M6809T_CC_H)) { cf |= 0x06; }
            if (msn > 0x80 && lsn > 9) { cf |= 0x60; }
            if (msn > 0x90 || (s->CC & M6809T_CC_C)) { cf |= 0x60; }
            t += cf;
            s->CC &= ~(M6809T_CC_N | M6809T_CC_Z | M6809T_CC_V);
            if (t & 0x100) s->CC |= M6809T_CC_C;
            s->A = (uint8_t)t;
            set_nz8(s, s->A);
            s->cycles = 2;
            break;
        ...
        case 0x30: /* LEAX */
            s->X = indexed_addr(s);
            s->CC &= ~M6809T_CC_Z;
            if (!s->X) s->CC |= M6809T_CC_Z;
            s->cycles += 4; break;

        case 0x31: /* LEAY */
            s->Y = indexed_addr(s);
            s->CC &= ~M6809T_CC_Z;
            if (!s->Y) s->CC |= M6809T_CC_Z;
            s->cycles += 4; break;
        ...
        case 0x3D: /* MUL */
            ...
            SET_D(s, val16);
            s->CC &= ~(M6809T_CC_Z | M6809T_CC_C);
            if (!val16) s->CC |= M6809T_CC_Z;
            if (s->B & 0x80) s->CC |= M6809T_CC_C;
            s->cycles = 11; break;
        ...
        case 0x86: s->A = FETCH8(s); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles = 2; break; /* LDA */
        ...
        case 0x8D: /* BSR */
            ...
            s->PC += offset;
            s->cycles = 7; break;
        case 0x8E: s->X = FETCH16(s); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles = 3; break; /* LDX imm */
        ...
        case 0x96: ea = direct_addr(s); s->A = READ8(s, ea); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles = 4; break;
        case 0x97: ea = direct_addr(s); WRITE8(s, ea, s->A); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles = 4; break; /* STA */
        ...
        case 0x9E: ea = direct_addr(s); s->X = READ16(s, ea); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        case 0x9F: ea = direct_addr(s); WRITE16(s, ea, s->X); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles = 5; break; /* STX */
        ...
        case 0xA6: ea = indexed_addr(s); s->A = READ8(s, ea); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles += 4; break;
        case 0xA7: ea = indexed_addr(s); WRITE8(s, ea, s->A); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles += 4; break;
        ...
        case 0xAE: ea = indexed_addr(s); s->X = READ16(s, ea); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles += 5; break;
        case 0xAF: ea = indexed_addr(s); WRITE16(s, ea, s->X); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles += 5; break;
        ...
        case 0xB6: ea = extended_addr(s); s->A = READ8(s, ea); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        case 0xB7: ea = extended_addr(s); WRITE8(s, ea, s->A); set_nz8(s, s->A); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        ...
        case 0xBE: ea = extended_addr(s); s->X = READ16(s, ea); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
        case 0xBF: ea = extended_addr(s); WRITE16(s, ea, s->X); set_nz16(s, s->X); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
        ...
        case 0xC6: s->B = FETCH8(s); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles = 2; break;
        ...
        case 0xCC: { uint16_t v = FETCH16(s); SET_D(s, v); set_nz16(s, v); s->CC &= ~M6809T_CC_V; } s->cycles = 3; break; /* LDD imm */
        case 0xCE: s->U = FETCH16(s); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles = 3; break; /* LDU imm */
        ...
        case 0xD6: ea = direct_addr(s); s->B = READ8(s, ea); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles = 4; break;
        case 0xD7: ea = direct_addr(s); WRITE8(s, ea, s->B); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles = 4; break;
        ...
        case 0xDC: ea = direct_addr(s); { uint16_t v = READ16(s,ea); SET_D(s,v); set_nz16(s,v); s->CC &= ~M6809T_CC_V; } s->cycles = 5; break;
        case 0xDD: ea = direct_addr(s); { uint16_t v = REG_D(s); WRITE16(s,ea,v); set_nz16(s,v); s->CC &= ~M6809T_CC_V; } s->cycles = 5; break;
        case 0xDE: ea = direct_addr(s); s->U = READ16(s, ea); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        case 0xDF: ea = direct_addr(s); WRITE16(s, ea, s->U); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        ...
        case 0xE6: ea = indexed_addr(s); s->B = READ8(s, ea); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles += 4; break;
        case 0xE7: ea = indexed_addr(s); WRITE8(s, ea, s->B); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles += 4; break;
        ...
        case 0xEC: ea = indexed_addr(s); { uint16_t v = READ16(s,ea); SET_D(s,v); set_nz16(s,v); s->CC &= ~M6809T_CC_V; } s->cycles += 5; break;
        case 0xED: ea = indexed_addr(s); { uint16_t v = REG_D(s); WRITE16(s,ea,v); set_nz16(s,v); s->CC &= ~M6809T_CC_V; } s->cycles += 5; break;
        case 0xEE: ea = indexed_addr(s); s->U = READ16(s, ea); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles += 5; break;
        case 0xEF: ea = indexed_addr(s); WRITE16(s, ea, s->U); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles += 5; break;
        ...
        case 0xF6: ea = extended_addr(s); s->B = READ8(s, ea); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        case 0xF7: ea = extended_addr(s); WRITE8(s, ea, s->B); set_nz8(s, s->B); s->CC &= ~M6809T_CC_V; s->cycles = 5; break;
        ...
        case 0xFC: ea = extended_addr(s); { uint16_t v = READ16(s,ea); SET_D(s,v); set_nz16(s,v); s->CC &= ~M6809T_CC_V; } s->cycles = 6; break;
        case 0xFD: ea = extended_addr(s); { uint16_t v = REG_D(s); WRITE16(s,ea,v); set_nz16(s,v); s->CC &= ~M6809T_CC_V; } s->cycles = 6; break;
        case 0xFE: ea = extended_addr(s); s->U = READ16(s, ea); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
        case 0xFF: ea = extended_addr(s); WRITE16(s, ea, s->U); set_nz16(s, s->U); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
        ...
        case 0x10:
            ...
            switch (op) {
                ...
                /* LDY */
                case 0x8E: s->Y = FETCH16(s); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles = 4; break;
                case 0x9E: ea = direct_addr(s); s->Y = READ16(s,ea); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
                case 0xAE: ea = indexed_addr(s); s->Y = READ16(s,ea); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles += 6; break;
                case 0xBE: ea = extended_addr(s); s->Y = READ16(s,ea); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles = 7; break;

                /* STY */
                case 0x9F: ea = direct_addr(s); WRITE16(s,ea,s->Y); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
                case 0xAF: ea = indexed_addr(s); WRITE16(s,ea,s->Y); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles += 6; break;
                case 0xBF: ea = extended_addr(s); WRITE16(s,ea,s->Y); set_nz16(s, s->Y); s->CC &= ~M6809T_CC_V; s->cycles = 7; break;
                ...
                /* LDS */
                case 0xCE: s->S = FETCH16(s); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->nmi_armed = 1; s->cycles = 4; break;
                case 0xDE: ea = direct_addr(s); s->S = READ16(s,ea); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->nmi_armed = 1; s->cycles = 6; break;
                case 0xEE: ea = indexed_addr(s); s->S = READ16(s,ea); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->nmi_armed = 1; s->cycles += 6; break;
                case 0xFE: ea = extended_addr(s); s->S = READ16(s,ea); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->nmi_armed = 1; s->cycles = 7; break;

                /* STS */
                case 0xDF: ea = direct_addr(s); WRITE16(s,ea,s->S); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->cycles = 6; break;
                case 0xEF: ea = indexed_addr(s); WRITE16(s,ea,s->S); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->cycles += 6; break;
                case 0xFF: ea = extended_addr(s); WRITE16(s,ea,s->S); set_nz16(s, s->S); s->CC &= ~M6809T_CC_V; s->cycles = 7; break;
